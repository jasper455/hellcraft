...
public class SmallGrenadeProjectileRenderer extends EntityRenderer<FlameBulletEntity> {
    private FireBulletProjectileModel model;

    public SmallGrenadeProjectileRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        model = new FireBulletProjectileModel(pContext.bakeLayer(ModModelLayers.BULLET));
    }


    @Override
    public void render(FlameBulletEntity pEntity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();
...
//                .renderSphere(vertexconsumer, poseStack, -50, 50, 50);
    ...
    @Override
    public ResourceLocation getTextureLocation(FlameBulletEntity bulletProjectileEntity) {
